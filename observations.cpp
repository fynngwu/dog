...
#include "stubs/wit_c_sdk.h"
#include "stubs/robstride.hpp"

// Reference to sReg defined in wit_c_sdk.c
extern volatile uint32_t sReg[64];
...
void IMUComponent::AutoScanSensor() {
    ...
    for(i = 0; i < sizeof(c_uiBaud)/sizeof(int); i++) {
        ...
        fd = serial_open(dev_path.c_str(), s_iCurBaud);
...
// ============================================================================
// Joint Component Implementation
// ============================================================================
JointComponent::JointComponent(int count,
                               std::shared_ptr<RobstrideController> ctrl,
                               const std::vector<int>& indices,
                               const std::vector<float>& offs)
    : count_(count), controller(ctrl), motor_indices(indices), offsets(offs) {}

// ============================================================================
// Action Component Implementation
// ============================================================================
ActionComponent::ActionComponent(int dim) : prev_actions(dim, 0.0f) {}

// ============================================================================
// Command Component Implementation
// ============================================================================
CommandComponent::CommandComponent(int dim, std::shared_ptr<Gamepad> gp)
    : command(dim, 0.0f), gamepad(gp) {}
